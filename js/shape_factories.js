...
CircleFactory.prototype.overlaps_image = function(img_data, circle) {
  ...
  var width = img_data.width / 3;
  var offset = width / 8;
  var radius = (width - (offset * 2)) / 2
  if (isInsideCircle(circle.x, circle.y, radius + offset, radius + offset, radius)) {
    return [1, 1]
  }
  // if (circle.x > 0 && (circle.x + (circle.radius * 2)) < limit && circle.y > 0 && (circle.y + (circle.radius * 2)) < limit) {
  //   return [100, 1]
  // }
...
isInsideCircle = function(a, b, x, y, r) {
